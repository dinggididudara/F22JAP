...
/**
 * Demonstrates how the JPanel works and how the opaque property of JPanel (and
 * JLabel) affects the visibility of the other components.
 * 
 * @version 1.19.1
...
public class OpacityDemo extends JFrame {
	...
	/**
	 * Creates a JPanel containing another JPanel containing a JLabel. The inner
	 * JPanel's opacity property is set according to the value of the first
	 * parameter. A JLabel is placed inside the inner JPanel to give it some
	 * content. The JLabel's opacity property is set according to the value of the
	 * first parameter, and the JLabel's text is set according to the value of the
	 * second parameter.
	 * 
	 * @param isOpaque  if true, sets the opacity of the JLabel to true; if false,
	 *                  sets the opacity of the inner JPanel to false.
...
//  outer.setBorder(BorderFactory.createLineBorder(Color.blue));
		outer.setBorder(
				new CompoundBorder(BorderFactory.createLineBorder(Color.blue), new EmptyBorder(10, 10, 10, 10)));
...
//  inner.setBorder(BorderFactory.createLineBorder(Color.red));
		inner.setBorder(new CompoundBorder(BorderFactory.createLineBorder(Color.red), new EmptyBorder(10, 10, 10, 10)));
		// change the opacity of the inner panel.
		if (!isOpaque)
			inner.setOpaque(isOpaque);
		// create and set the label
		JLabel label = new JLabel(labelText);
		label.setToolTipText("Label");
		label.setBorder(new CompoundBorder(BorderFactory.createRaisedBevelBorder(), new EmptyBorder(10, 10, 10, 10)));
